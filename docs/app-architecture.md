...
Google OAuth는 인증 전용이다. 랭킹, 공유, 그룹 화면에는 `profiles.nickname`만 노출한다. 이메일은 API 응답과 UI에 노출하지 않는다.

### 익명 플레이 허용

로그인하지 않은 사용자는 오늘 문제를 풀 수 있다. 단, 랭킹 등록, 그룹 참여, 1등 확성기 메시지는 로그인과 닉네임 설정을 요구한다.
...
로그인하지 않아도 오늘 문제를 풀 수 있지만, 랭킹 등록과 그룹 참여는 로그인 후 가능하다는 상태만 보여준다.
...
- 로그인 사용자는 `user_id` 저장
- 비로그인 사용자는 httpOnly cookie 기반 anonymous session 사용
