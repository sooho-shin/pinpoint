...
- 커스텀 게임 플레이어와 데일리 게임 비로그인 플레이어가 닉네임으로 랭킹에 등록할 수 있다.
...
오늘 문제 진행 상태는 로그인 사용자는 `publication_id + user_id`, 비로그인 사용자는 `publication_id + anonymous_session_id` 기준으로 조회한다. 익명 세션은 서버가 발급한 httpOnly 쿠키로만 관리하고, 클라이언트 JS에서 읽을 수 없게 한다. 랭킹 등록은 로그인 사용자 프로필 또는 익명 닉네임으로 가능하다. 그룹 참여와 1등 확성기 메시지는 로그인과 닉네임 설정을 요구한다.
...
익명 성공 attempt는 로그인하지 않아도 닉네임 입력으로 오늘의 랭킹에 등록할 수 있다. 이 경우 `leaderboard_entries.user_id`는 비어 있을 수 있고, 서버가 관리하는 `anonymous_session_id`와 `nickname_snapshot`으로 최초 기록을 고정한다. 같은 `publication_id + anonymous_session_id` 조합은 최초 visible/flagged 기록만 허용한다. 닉네임 미입력 상태에서는 결과만 볼 수 있고 랭킹에는 등록하지 않는다.

### 커스텀 게임

커스텀 게임은 데일리 publication과 분리된 별도 흐름이다.

- 제작 화면은 `/custom/new`다.
- 플레이 화면은 `/custom/[slug]`다.
- 커스텀 랭킹 화면은 `/custom/[slug]/ranking`이다.
- 관리 화면은 `/custom/manage/[token]`이다.
- 플레이 링크 slug와 관리 token은 추측하기 어려운 난수여야 한다.
- 정답과 단서 5개는 생성 후 수정하지 않는다.
- 관리 링크는 숨김과 삭제만 제공한다.
- 숨김 또는 삭제 상태의 게임은 플레이 API와 공개 화면에서 정답/단서를 노출하지 않는다.
- 커스텀 게임 결과와 랭킹은 데일리 랭킹과 독립적으로 저장한다.
- 같은 브라우저의 같은 커스텀 게임은 최초 완료 기록만 랭킹에 인정한다.
- 플레이어 신고는 별도 row로 저장하고, 운영 상태는 `active`, `hidden`, `deleted`로 관리한다.

커스텀 게임 API도 데일리 게임과 같은 보안 원칙을 따른다. terminal result 전에는 정답, aliases, rationale, 잠긴 단서를 반환하지 않는다.
...
- 그룹 URL로 들어온 경우 해당 공유 그룹의 랭킹과 URL 복사 액션
- visible 랭킹 목록
- 내 기록 강조
- 로그인 사용자라면 내 기록 강조
...
- `/?group={invite_code}`는 초대 코드에 해당하는 오늘 공유 그룹을 활성화한다.
- `/ranking?group={invite_code}`는 같은 공유 그룹의 오늘 랭킹을 표시한다.
- 별도의 그룹 만들기 UI는 두지 않는다. 메인 공유 버튼이 그룹 URL을 암묵 생성하고 URL만 복사한다.
- 그룹 URL 생성은 로그인 없이도 가능하다.
- 그룹 참여와 랭킹 노출은 로그인 + 닉네임 설정을 요구한다.
- 그룹은 활성 공개일의 publication에만 유효하다.
- 공유 URL로 들어온 로그인 사용자는 해당 그룹 멤버로 upsert된다.
...
- 정규화 후 완전 일치 또는 alias 완전 일치를 우선 인정
- 정규화 후 5자 이상인 답변은 `1 - 편집거리 / 최대길이 >= 0.95`이면 오타 보정으로 인정
- 5자 미만의 짧은 답변은 한 글자 차이로 의미가 바뀔 수 있으므로 fuzzy matching을 적용하지 않음
- 서버 기준 `elapsed_ms` 계산
